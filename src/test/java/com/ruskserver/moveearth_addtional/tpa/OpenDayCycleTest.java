...
class OpenDayCycleTest {
    ...
    void switchesOpeningDayAtNineteenJst() {
        assertEquals("2026-09-02", OpenDayCycle.currentId(clockAt(2026, 9, 3, 18, 59)));
        assertEquals("2026-09-03", OpenDayCycle.currentId(clockAt(2026, 9, 3, 19, 0)));
    }

    @Test
    void reportsNextNineteenOClockReset() {
        assertEquals(
                ZonedDateTime.of(2026, 9, 3, 19, 0, 0, 0, OpenDayCycle.JST),
                OpenDayCycle.nextReset(clockAt(2026, 9, 3, 18, 59)));
        assertEquals(
                ZonedDateTime.of(2026, 9, 4, 19, 0, 0, 0, OpenDayCycle.JST),
                OpenDayCycle.nextReset(clockAt(2026, 9, 3, 19, 0)));
    }

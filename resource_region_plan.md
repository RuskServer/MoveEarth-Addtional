...
**地図オーバーレイは実装しない (user判断、2026-09-21)。** 完了条件の
「地図で境界が見える」はこの項目については満たさないものとする。

領土オーバーレイ（`TerritoryMapRenderer`、2026-09-13）と性質が違う。

| | 何を示すか | 見えないと |
|---|---|---|
| 領土 | **ルールの境界**。建てられない、攻められる | 事故が起きる |
| 地方 | 資源の傾向 | 掘れば分かる |

領土は消せてはいけない情報なので、常時描画でよい（切替キーが既定未割当なのは
**意図どおり**であり、不具合ではない）。地方は消せてよい情報であり、
消せるようにすると今度は「どちらのキーか分からない」を生む。

そして**地方オーバーレイが足す情報は「どこからどこまでか」だけ**で、
「なぜここで掘れないのか」は境界通知とHubタブで既に答えている。
歩けば分かることのために、全員の地図を常時塗る価値はない。

再検討する条件: 地方境界が地形から読み取れず、プレイヤーが
「どこまで行けば変わるのか」を繰り返し尋ねるようになった場合。
そのときは色面ではなく**境界線のみ**を検討する。色面は地形を潰す。
